...
flags.DEFINE_string('method', 'PGD', 'attack method, PGD,SPEC or CW')
...
original_support = copy.deepcopy(support)  # 保存原始L
...
feed_dict.update({placeholders['ss']: np.zeros(n_node)})
...
if os.path.exists('label_' + FLAGS.dataset + '.npy'):
    ...
    ret = np.argmax(ret, 1)  # 得到结果n个预测值？
    label = np.zeros_like(label)  # label大小的全0阵
    label[np.arange(label.shape[0]), ret] = 1  # label变为了一个n*7的矩阵，只有标签位是1，其他为0
    np.save('label_' + FLAGS.dataset + '.npy', label)
feed_dict.update({placeholders['labels']: label})  # 此处更新了label
...
for epoch in range(FLAGS.steps):
    ...
    if FLAGS.method == 'CW':
        ...
        a, support, S, g = sess.run([model.a, model.placeholders['support'], model.upper_S_real, model.Sgrad],
                                    feed_dict=feed_dict)  # a是按照梯度方向更新后的扰动，原有拉普拉斯矩阵，
    elif FLAGS.method == 'SPEC':
        a, support, S, g, S2 = sess.run([model.a, model.placeholders['support'], model.upper_S_real, model.Sgrad,
                                         model.S2], feed_dict=feed_dict)
        feed_dict.update({placeholders['ss']: S2[0]})
    else:
        raise ValueError('invalid attack method: {}'.format(FLAGS.method))
    upper_S_update = bisection(a, eps, xi)
    #print(np.count_nonzero(np.array(upper_S_update[0])))
...
# support is the disturbing matrix
