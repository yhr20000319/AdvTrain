...
flags.DEFINE_integer('train_steps', 120, 'Number of steps to train')
...
flags.DEFINE_float('perturb_ratio', 0.2, 'perturb ratio of total edges.')
...
save_name = 'p_0.1_r_1_FPAT_' + FLAGS.dataset
...
placeholders = {
    ...
    'ori_support': [tf.placeholder(tf.float32) for _ in range(num_supports)],
    ...
    'ss': tf.placeholder(tf.float32, shape=n_node),
}

# Create model
# for non sparse
model = model_func(placeholders, input_dim=features.shape[1], attack='PGD', FPAT=True, adv_rate=1, logging=False)

# Initialize session
sess = tf.Session()


def evaluate(features, support, ori_support, labels, mask, placeholders):
    t_test = time.time()
    feed_dict_val = construct_feed_dict(features, support, labels, mask, placeholders)
    feed_dict_val.update({placeholders['support'][i]: support[i] for i in range(len(support))})
    feed_dict_val.update({placeholders['ori_support'][i]: ori_support[i] for i in range(len(ori_support))})
    outs_val = sess.run([model.attack_loss, model.accuracy], feed_dict=feed_dict_val)
    return outs_val[0], outs_val[1], (time.time() - t_test)


# Init variables 此时拿到干净训练的参数
sess.run(tf.global_variables_initializer())

model.load("./nat_cora", sess=sess)
...
for n in range(FLAGS.train_steps):
    ...
    train_feed_dict.update({placeholders['ori_support'][i]: nat_support[i] for i in range(len(nat_support))})
    ...
    attack_feed_dict.update({placeholders['ori_support'][i]: nat_support[i] for i in range(len(adv_support))})
    ...
    new_adv_support = attack.perturb(attack_feed_dict, FLAGS.discrete, attack_label, attack_label_mask, FLAGS.att_steps,
                                     ori_support=nat_support)

    train_loss, train_acc, _ = attack.evaluate(new_adv_support, nat_support, y_train, train_mask)
    test_loss_adv, test_acc_adv, _ = attack.evaluate(new_adv_support, nat_support, y_test, test_mask)
    print('[adv support] train acc: {}, train loss: {}, test acc: {}, test loss: {}'.format(train_acc, train_loss,
                                                                                            test_acc_adv,
                                                                                            test_loss_adv))
    train_loss, train_acc, _ = attack.evaluate(nat_support, nat_support, y_train, train_mask)
    test_loss_nat, test_acc_nat, _ = attack.evaluate(nat_support, nat_support, y_test, test_mask)
    print('[nat support] train acc: {}, train loss: {}, test acc: {}, test loss: {}'.format(train_acc, train_loss,
                                                                                            test_acc_nat,
                                                                                            test_loss_nat))
...
# df2 = pd.read_csv("A‘_0.3.csv")
# df2 = df2.drop('Unnamed: 0', axis=1)
# new_adv_support[0] = origin_support
# new_adv_support[0] = df2.values
test_cost, test_acc, test_duration = evaluate(features, new_adv_support, nat_support, y_train, train_mask, placeholders)
print("Train set results:", "cost=", "{:.5f}".format(test_cost),
      "accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
test_cost, test_acc, test_duration = evaluate(features, new_adv_support, nat_support, y_val, val_mask, placeholders)
print("Validation set results:", "cost=", "{:.5f}".format(test_cost),
      "accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
test_cost, test_acc, test_duration = evaluate(features, new_adv_support, nat_support, y_test, test_mask, placeholders)
...
print("in origin:")
test_cost, test_acc, test_duration = evaluate(features, nat_support, nat_support, y_train, train_mask, placeholders)
print("Train set results:", "cost=", "{:.5f}".format(test_cost),
      "accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
test_cost, test_acc, test_duration = evaluate(features, nat_support, nat_support, y_val, val_mask, placeholders)
print("Validation set results:", "cost=", "{:.5f}".format(test_cost),
      "accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
test_cost, test_acc, test_duration = evaluate(features, nat_support, nat_support, y_test, test_mask, placeholders)
print("Test set results:", "cost=", "{:.5f}".format(test_cost),
      "accuracy=", "{:.5f}".format(test_acc), "time=", "{:.5f}".format(test_duration))
