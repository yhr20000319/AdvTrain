...
from utils import construct_feed_dict, bisection, filter_potential_singletons, load_data
...
class PGDAttack:
    ...
    #
    def evaluate(self, support, ori_support, labels, mask):
        t_test = time.time()
        ...
        feed_dict_val.update({self.model.placeholders['ori_support'][i]: ori_support[i] for i in range(len(ori_support))})
            ...
            if discrete:  # 最后需要一个离散矩阵
                ...
                if epoch == k - 1:
                    ...
                    for i in range(10):
                        ...
                        cost, acc, duration = self.evaluate(support_d, ori_support, y_test, test_mask)
                    ...
                    break
            cost, acc, duration = self.evaluate(support, ori_support, y_test, test_mask)
